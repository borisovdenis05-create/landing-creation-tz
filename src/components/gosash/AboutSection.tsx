...
const FALLBACK_BRANCH_CARDS = [
  { id: 1, name: "Филиал на Гагарина", addr: "ул. Гагарина, 20А, Симферополь", rating: 5.0, map_url: "https://yandex.ru/maps/-/CPBAb-1K" },
  { id: 2, name: "Филиал на Киевской", addr: "Киевская ул., 41, Симферополь", rating: 5.0, map_url: "https://yandex.ru/maps/-/CPBAfQLi" },
  { id: 3, name: "Филиал на Залесской", addr: "Залесская ул., 121, Симферополь", rating: 4.8, map_url: "https://yandex.ru/maps/-/CPBAjU2Y" },
  { id: 4, name: "Филиал на Самокиша", addr: "ул. Самокиша, 4, Симферополь", rating: 4.9, map_url: "https://yandex.ru/maps/-/CPBAnA6k" },
  { id: 5, name: "Филиал на Лермонтова", addr: "ул. Лермонтова, 13А, Симферополь", rating: 5.0, map_url: "https://yandex.ru/maps/-/CPBArZ88" },
];

const FALLBACK_STATS: DbStat[] = [
  { id: 1, value: "94,5%", label: "учеников сдают теорию в ГИБДД с первого раза", icon: "GraduationCap" },
  { id: 2, value: "19", label: "лицензированных инструкторов в штате", icon: "Users" },
  { id: 3, value: "1,9", label: "попытки нужно в среднем для сдачи вождения в городе", icon: "Car" },
  { id: 4, value: "356 лет", label: "общий опыт работы инструкторов", icon: "Award" },
  { id: 5, value: "9,4/10", label: "оценка качества услуг учениками", icon: "Star" },
...
export default function AboutSection() {
  const { items: branchesDb } = usePublicList<DbBranch>("public-branches");
  const { items: statsDb } = usePublicList<DbStat>("public-stats");
  ...
  return (
    <>
          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="mb-10">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                {branches.map((branch) => (
                  <div key={branch.id} className="border border-white/10 rounded-2xl p-5 hover:border-orange-400 hover:shadow-md transition-all" style={{ background: "#2e2e2e" }}>
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0 border border-white/15 rounded-lg px-2 py-1" style={{ background: "#222222" }}>
                        ...
                        <span className="font-black text-sm text-white">{Number(branch.rating).toFixed(1)}</span>
      ...
      {settings.block_stats !== "false" && (
        <section className="py-20" style={{ background: "#2e2e2e" }}>
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-black text-white mb-3 uppercase">{statsTitle}</h2>
              ...
              {stats.map((stat) => (
                ...
                >
                  <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center mb-3">
                    <Icon name={stat.icon as Parameters<typeof Icon>[0]["name"]} size={22} className="text-white" fallback="Star" />
                  </div>
                  <p className="text-2xl md:text-3xl font-black text-white mb-1">{stat.value}</p>
                  <p className="text-white/60 text-xs md:text-sm leading-relaxed">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
